    ...
    <div class="">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h4 class="card-title">Form Tambah Tantangan Baru</h4>
                <div>
                ...
                <form action="{{ route('challenge.store') }}" method="POST">
                    ...
                    <div class="form-group my-2">
                        ...
                        <input type="text" name="title" id="title"
                            ...
                            placeholder="Contoh: Pemburu Lapangan Lumajang">
                        @error('title')
                        ...
                        <select name="challenge_type_id" id="challenge_type_id"
                            ...
                            @foreach ($challengeTypes as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('challenge_type_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }} ({{ $type->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 form-group my-2">
                            ...
                            <input type="number" name="total_winner" id="total_winner"
                                ...
                                value="{{ old('total_winner', 3) }}" min="1">
                            @error('total_winner')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                    ...
                    <div class="form-group my-2">
                        ...
                        <textarea name="description" id="description" rows="4"
                            ...
                            placeholder="Tuliskan detail reward tambahan atau syarat mutlak tantangan...">{{ old('description') }}</textarea>
                        @error('description')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="form-group my-2">
                        ...
                        <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                            <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>Draft (Simpan Internal)
                            </option>
                            <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Active (Langsung Rilis
                                ke User)</option>
                            <option value="finished" {{ old('status') == 'finished' ? 'selected' : '' }}>Finished (Tutup
                                Kompetisi)</option>
                        </select>
                        @error('status')
                            <small class="text-danger">{{ $message }}</small>
                    ...
                    <div class="my-3 d-flex justify-content-end">
                        <button type="submit" class="btn btn-primary px-4">Simpan Tantangan</button>
                    </div>
